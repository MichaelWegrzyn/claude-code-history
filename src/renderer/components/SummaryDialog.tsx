    ...
    try {
      ...
      if ('showSaveFilePicker' in window) {
        const fileHandle = await (window as any).showSaveFilePicker({
          ...
          suggestedName: `conversation-summary-${sessionId.slice(0, 8)}.md`,
        });
        
        const writable = await fileHandle.createWritable();
        await writable.write(summary.markdown);
        await writable.close();
        setToastMessage('Summary exported successfully!');
        setShowToast(true);
      } else {
        // Fallback: copy to clipboard
        await navigator.clipboard.writeText(summary.markdown);
        setToastMessage('Summary copied to clipboard! (File saving not supported)');
        setShowToast(true);
      }
    } catch (error) {
      ...
      try {
        await navigator.clipboard.writeText(summary.markdown);
        setToastMessage('Export cancelled, but summary copied to clipboard!');
        setShowToast(true);
      } catch (clipboardError) {
        setToastMessage('Export failed and clipboard access denied');
        setShowToast(true);
      }
    }
  };
  ...
  return (
    <Dialog.Root open={isOpen} onOpenChange={onClose}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-50 w-full max-w-4xl max-h-[85vh] translate-x-[-50%] translate-y-[-50%] bg-background border rounded-lg shadow-lg overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4">
                      ...
                      <ul className="space-y-1">
                        {summary.keyDecisions.map((decision, index) => (
                          <li key={index} className="text-sm">
                            <span className="text-primary font-medium">{index + 1}.</span> {decision}
                          </li>
                        ))}
                      </ul>
                    </div>
                    ...
                    <div>
                      ...
                      <ul className="space-y-1">
                        {summary.filesModified.map((file, index) => (
                          <li key={index} className="text-sm">
                            <code className="bg-muted px-1 py-0.5 rounded text-xs">{file}</code>
                          </li>
                        ))}
                      </ul>
                    </div>
            ...
            {summary && activeTab === 'markdown' && (
              <div className="h-full">
                <pre className="whitespace-pre-wrap font-mono text-xs bg-muted p-4 rounded-lg h-full overflow-auto">
                  {summary.markdown}
                </pre>
              </div>
            )}
